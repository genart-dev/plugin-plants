...
/** Seasonal color modifiers for deciduous plants. */
export function seasonalModify(
  palette: SeasonalPalette,
  season: "spring" | "summer" | "autumn" | "winter" | "evergreen",
): SeasonalPalette {
  switch (season) {
    case "spring":
      return { ...palette, leaf: lighten(palette.leaf, 0.3) };
    case "summer":
      return palette;
    case "autumn":
      return { ...palette, leaf: "#C0582A" };
    case "winter":
      return { ...palette, leaf: darken(palette.leaf, 0.3) };
    case "evergreen":
      return palette;
  }
}
